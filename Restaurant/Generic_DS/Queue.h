...
public :
	...
	bool enqueue(const T& newEntry,int id);
...
template <typename T>
bool Queue<T>::enqueue( const T& newEntry,int id)
{
	Node<T>* newNodePtr = new Node<T>(newEntry,id);
	// Insert the new node
	if (isEmpty())
		frontPtr = newNodePtr; // The queue is empty
	else
...
bool Queue<T>:: dequeue(T& frntEntry)  
{
	...
	if (frontPtr==nullptr)	 // Special case: last node in the queue
		backPtr = nullptr;
