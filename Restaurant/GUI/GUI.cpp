...
void GUI::PrintMultiMessage(string msg, int line) const	//Prints a message on status bar
{
	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.1) + (line - 1) * 20, msg); // You may need to change these coordinates later 
																							 // to be able to write multi-line
}
...
void GUI::AddOrderForDrawing(Order* ptr)
{
	if (OrderCount < MaxPossibleOrdCnt)
	{
		if (ptr->GetType() == TYPE_VIP)
		{
			...
			{
				if (ptr->GetPriorityIndicator() <= OrdListForDrawing[VipCount - 1]->GetPriorityIndicator())
				{
					for (int i = OrderCount; i > VipCount - 1; i--)
						OrdListForDrawing[i] = OrdListForDrawing[i - 1];
					OrdListForDrawing[VipCount] = ptr;
				}
				else
				{
					for (int i = 0; i < VipCount; i++)
					{
						if (ptr->GetPriorityIndicator() > OrdListForDrawing[i]->GetPriorityIndicator())
						{
							for (int j = OrderCount; j > i; j--)
		...
		else if (ptr->GetType() == TYPE_FROZ)
		{
			for (int i = OrderCount; i > VipCount + FrozenCount; i--)
				OrdListForDrawing[i] = OrdListForDrawing[i - 1];
			OrdListForDrawing[VipCount + FrozenCount] = ptr;
			FrozenCount++;
		}
		else
		{
...
void GUI::DeleteOrder(Order* ptr)
{
	for (int i = 0; i < OrderCount; i++)
	{
		if (OrdListForDrawing[i] == ptr)
		{
